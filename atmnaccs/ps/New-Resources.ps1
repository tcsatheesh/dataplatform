...
function Set-ResourceSpecificParameters {
    ...
    if ($resourceparam.name -eq "variableValues") {
        $val = Get-VariableValues -value $resourceparam.value
    }
    elseif ($resourceparam.name -eq "webhookUri") {        
        $val = Get-WebhookUri -resourcename $resource.name
    }
    else {
