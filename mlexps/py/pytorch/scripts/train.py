...
data_root = os.path.join(data_folder, "hymenoptera_data")
...
imshow(out, title=[class_names[x] for x in classes], filename="sample")
    ...
    with torch.no_grad():
        for i, (inputs, labels) in enumerate(dataloaders['val']):
            ...
            for j in range(inputs.size()[0]):
                ...
                imshow(inputs.cpu().data[j],filename="predicted_{0}".format(j))
