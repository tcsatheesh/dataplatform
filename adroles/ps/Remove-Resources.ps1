                ...
                if ($principalObj -ne $null) {
                    ...
                    if ([string]::IsNullOrEmpty($objectid)) {
                        ...
                        if ($app -ne $null) {
                            ...
                            if ($member -eq $null) {                                
                                Write-Verbose "$objectId does not exist in role $roleName"
                            }
                            else {
                                Write-Verbose "$objectId already exists in role $roleName. Removing..."
                                Remove-AzureADDirectoryRoleMember -ObjectId $roleObjectId -MemberId $objectid
                                Write-Verbose "$objectId removed from role $roleName."
                            }
                        }
                        else {
        ...
        elseif ($member.type -eq "users") {
            ...
            if ($user -ne $null) {
                ...
                if ($member -eq $null) {                    
                    ...
                    Write-Verbose "$objectId exists in role $roleName. Removing..."
                    Remove-AzureADDirectoryRoleMember -ObjectId $roleObjectId -MemberId $objectid
                    Write-Verbose "$objectId removed in role $roleName."
                }
            }else {
