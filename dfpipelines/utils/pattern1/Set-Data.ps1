...
function Zip-It {
    $source = "{0}\{1}" -f $PSScriptRoot, $sourcedir
    $datetimestamp = (Get-Date).AddHours($addHours).ToString("yyyyMMddHHmm")
    $destinationFileName = "{0}_{1}.zip" -f $sourcedir, $datetimestamp
    ...
    $null = [io.compression.zipfile]::CreateFromDirectory($Source, $destination)
